        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1.5rem;">
                ...
                <table class="custom-table">
                    ...
                    <tbody>
                        @foreach($employeesByDept as $dept)
                            <tr>
                                <td style="font-weight: 700; color: #fff;">{{ $dept->name }}</td>
                                <td>{{ $dept->headOfDepartment->full_name ?? 'N/A' }}</td>
                                <td><strong style="color: #34d399;">{{ $dept->employees_count }} Members</strong></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="glass-panel" style="padding: 1.5rem;">
                ...
                <table class="custom-table">
                    ...
                    <tbody>
                        @foreach($leavesSummary as $ls)
                            <tr>
                                <td style="font-weight: 700; color: #818cf8;">{{ $ls->leaveType->name ?? 'General' }}</td>
                                <td>{{ $ls->total_requests }} Requests</td>
                                <td><strong style="color: #fbbf24;">{{ $ls->total_days_taken }} Days</strong></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="glass-panel" style="padding: 1.75rem;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem; margin-bottom: 1.5rem;">
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    ...
                    <div style="font-size: 1.5rem; font-weight: 800; color: #fff;">${{ number_format($totalBasic, 2) }}</div>
                </div>
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    <div style="font-size: 0.8rem; color: var(--text-muted);">Total Allowances & Overtime</div>
                    <div style="font-size: 1.5rem; font-weight: 800; color: #34d399;">+${{ number_format($totalAllowances, 2) }}</div>
                </div>
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    <div style="font-size: 0.8rem; color: var(--text-muted);">Income Tax (PAYE)</div>
                    <div style="font-size: 1.5rem; font-weight: 800; color: #f87171;">-${{ number_format($totalTax, 2) }}</div>
                </div>
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    <div style="font-size: 0.8rem; color: var(--text-muted);">Pension Fund (8%)</div>
                    <div style="font-size: 1.5rem; font-weight: 800; color: #f87171;">-${{ number_format($totalPension, 2) }}</div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem;">
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    ...
                    <div style="font-size: 1.5rem; font-weight: 800; color: #f87171;">-${{ number_format($totalSocialSecurity, 2) }}</div>
                </div>
                <div style="background: rgba(15,23,42,0.6); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid var(--bg-card-border);">
                    <div style="font-size: 0.8rem; color: var(--text-muted);">Medical Aid (3%)</div>
                    <div style="font-size: 1.5rem; font-weight: 800; color: #f87171;">-${{ number_format($totalMedicalAid, 2) }}</div>
                </div>
                <div style="background: linear-gradient(135deg, rgba(99,102,241,0.3), rgba(16,185,129,0.3)); padding: 1.25rem; border-radius: var(--radius-sm); border: 1px solid rgba(99,102,241,0.5); grid-column: span 2;">
                    ...
                    <div style="font-size: 1.85rem; font-weight: 900; color: #34d399;">${{ number_format($totalNetPay, 2) }}</div>
                </div>
            </div>
        </div>
    </div>
