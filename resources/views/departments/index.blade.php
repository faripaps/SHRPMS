            ...
            <div class="glass-panel" style="padding: 1.5rem; display: flex; flex-direction: column; justify-content: space-between;">
                <div>
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.85rem;">
                        <span class="badge" style="background: rgba(99, 102, 241, 0.2); color: #818cf8; font-size: 0.8rem;">{{ $dept->code }}</span>
                        <span style="font-size: 0.8rem; color: var(--text-muted);">{{ $dept->branch }}</span>
                    </div>

                    <h3 style="font-size: 1.2rem; font-weight: 800; color: #fff; margin-bottom: 0.35rem;">{{ $dept->name }}</h3>
                    <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.4; margin-bottom: 1rem;">{{ $dept->description }}</p>
                    ...
                    <div style="background: rgba(15, 23, 42, 0.5); padding: 0.85rem; border-radius: var(--radius-sm); margin-bottom: 1rem; border: 1px solid var(--bg-card-border); font-size: 0.85rem;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 0.35rem;">
                            ...
                            <strong style="color: #fff;">{{ $dept->headOfDepartment->full_name ?? 'Unassigned' }}</strong>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
                            <span style="color: var(--text-muted);">Staff Members:</span>
                            <strong style="color: #34d399;">{{ $dept->employees->count() }} Employees</strong>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--bg-card-border); padding-top: 0.85rem;">
                    ...
                    <strong style="color: #fbbf24; font-size: 1.05rem;">${{ number_format($dept->budget, 2) }}</strong>
                </div>
            </div>
        @endforeach
    </div>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    @foreach($positions as $pos)
                        <tr>
                            <td style="font-weight: 600; color: #818cf8;">{{ $pos->code }}</td>
                            <td style="font-weight: 700; color: #fff;">{{ $pos->title }}</td>
                            <td>{{ $pos->department->name ?? 'N/A' }}</td>
                            <td><span class="badge badge-overtime">{{ $pos->salary_grade }}</span></td>
                            <td style="color: #34d399;">${{ number_format($pos->min_salary, 2) }}</td>
                            <td style="color: #34d399;">${{ number_format($pos->max_salary, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
