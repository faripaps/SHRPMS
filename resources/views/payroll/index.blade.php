...
<div style="display: flex; flex-direction: column; gap: 1.5rem;" x-data="{ showPayrollModal: false }">
    ...
    <div x-show="showPayrollModal" style="position: fixed; inset: 0; background: rgba(0,0,0,0.8); z-index: 100; display: flex; align-items: center; justify-content: center; padding: 1rem;" x-cloak>
        <div class="glass-panel" style="width: 100%; max-width: 500px; padding: 2rem;" @click.away="showPayrollModal = false">
            ...
            <form action="{{ route('payroll.process') }}" method="POST" style="display: flex; flex-direction: column; gap: 1.25rem;">
                @csrf
                ...
                <div style="display: flex; justify-content: flex-end; gap: 1rem; margin-top: 1rem;">
                    ...
                    <button type="submit" class="btn btn-primary">Process Batch ({{ $employeesCount }} Staff)</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <table class="custom-table">
            ...
            <tbody>
                @forelse($runs as $run)
                    <tr>
                        <td>
                            <strong style="color: #818cf8; font-size: 0.95rem;">{{ $run->batch_reference }}</strong>
                            <div style="font-size: 0.75rem; color: var(--text-muted);">By: {{ $run->processed_by }}</div>
                        </td>
                        <td style="font-weight: 600; color: #fff;">
                            {{ date('F Y', mktime(0, 0, 0, $run->payroll_month, 1, $run->payroll_year)) }}
                        </td>
                        <td>
                            <strong style="color: #fff;">{{ $run->total_employees }} Employees</strong>
                        </td>
                        <td>
                            <span style="color: #fff; font-weight: 600;">${{ number_format($run->total_gross_pay, 2) }}</span>
                        </td>
                        <td>
                            <span style="color: #f87171; font-weight: 600;">-${{ number_format($run->total_deductions, 2) }}</span>
                        </td>
                        <td>
                            <strong style="color: #34d399; font-size: 1.05rem;">${{ number_format($run->total_net_pay, 2) }}</strong>
                        </td>
                        <td>
                            <span class="badge badge-approved">{{ $run->status }}</span>
                        </td>
                        <td style="text-align: right;">
                            <a href="{{ route('payroll.show', $run->id) }}" class="btn btn-secondary" style="padding: 0.4rem 0.85rem; font-size: 0.8rem;">
